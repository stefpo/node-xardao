...
class Connection {
    ...
    openCB(...args) {
        ...
        if (args.length == 1 ) {
            ...
            throw Error("openCB accepts 1 or 2 parameters")
        }
        try {
            if (! this.isOpen ) {
                this.db =  new sq3.Database(this.database);
                this.debug('open','Opening connection for SQLite3')
            } else {
                this.debug('open','Re-using connection for SQLite3')
            }
            this.isOpen = true
            this.isInUse = true
            then(callback);
    ...
    hardCloseCB(callback) {
        try {
            this.debug('hard close','Closing connection for SQLite3')
            if ( isOpen ) {
                this.db.close()
            }
            this.isInUse = false
    ...
    closeCB(callback) {
        if (this.pooled) {
            this.debug('Soft close','Closing connection for SQLite3')
            this.isInUse = false
            this.lastUse = new Date(Date.now())
            then(callback);
        } else {
            this.hardCloseCB(callback)
        }
    }
